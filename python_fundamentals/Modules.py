...
"""
1. Math module
   Used for calculations in scientific apps, finance, games, and simulations.
   Examples:
   - Calculating interest rates
   - Trigonometry in graphics or game physics
   - Factorials/combinations in probability apps or data science

2. Random module
   Essential for anything involving randomness or selection.
   Examples:
   - Random passwords or OTP generators
   - Simulating dice rolls or game events
   - Randomized testing (unit tests)
   - Shuffling playlists

3. Datetime module
   Almost every app uses this for time tracking.
   Examples:
   - Scheduling apps, calendars, reminders
   - Logging system events with timestamps
   - Calculating durations, deadlines, and expiry dates

4. OS module
   Helps interact with the file system and environment.
   Examples:
   - Reading/writing files dynamically
   - Automated scripts for backups, deployment, or folder organization
   - Traversing directories to batch process files

5. Sys module
   Useful for Python environment and CLI tools.
   Examples:
   - Reading command-line arguments for scripts
   - Controlling recursion limits in complex algorithms
   - Getting Python version or installation paths for cross-platform apps

6. Re module (Regular Expressions)
   Critical for text processing and data validation.
   Examples:
   - Validating emails, passwords, phone numbers
   - Extracting patterns from logs or scraped web data
   - Replacing or formatting text in bulk

7. Itertools module
   For efficient looping, combinatorics, and data pipelines.
   Examples:
   - Generating permutations or combinations in games, testing, or AI
   - Merging multiple lists or datasets
   - Infinite iterators for simulations or event streams

8. Functools module
   Makes functions more powerful and efficient.
   Examples:
   - reduce() → processing lists in one line
   - lru_cache() → optimizing recursive functions like Fibonacci
   - partial() → pre-filling function arguments for convenience

9. JSON module
   The backbone of data interchange in web apps and APIs.
   Examples:
   - Sending/receiving data between frontend and backend (AJAX, REST APIs)
   - Saving configuration or state in files
